...
$("<div id='container'>");
 $("<div id='title'><a  href='" + window.location.protocol + "//" + window.location.hostname+window.location.pathname+"'>Наш альбом</a> &gt; "+ j.feed.title.$t +"&nbsp;&nbsp;["+np+" Фото] </div>");
...
if (is_video == 1) { current_index_text = current_index_text + "&nbsp;&nbsp;[VIDEO]"; }  //show in the breadcrumbs that the item is a video
 ...
 $("<div id='container'>");
  $("<div id='title'><a href='"+ window.location.protocol + "//" + window.location.hostname+window.location.pathname+"'>Наш альбом</a> &gt; <a href='" + album_base_path + "' > " + my_fixed_galleryname + "</a> &gt; <!--" + img_filename + "-->" + current_index_text + "</div>");
...
if (n1 == null) //we're at the last picture in the album; going forward takes us to the album index
 ...
 $("<div id='navigation'>");
	if (photo_array.length > 1) { $("<a class='standard' href='"+prev+"'> Пред. </a>&nbsp; &nbsp;"); }
 	$("&nbsp; &nbsp;<a href='"+album_base_path+"'>Назад в альбом</a>&nbsp; &nbsp;");
	 if (photo_array.length > 1) { $("&nbsp; &nbsp;<a href='"+next+"'>След.</a> "); }
 $("</div> ");
...
// $("<center><table border=0><tr valign=top>");
 ...
 var max_width = 700; //max display width for our photos
